...
def epochs_perform_predictions(epochs, dataset_fn, show_all, target_value):
    shuffle_dataset(dataset_fn)
    # for i in range(0, epochs):
    for i in [0]:
        if check_if_emb_exists(dataset_fn, i):
            results = epoch_perform_prediction(i, dataset_fn, show_all, target_value)
...
def epoch_perform_prediction(epochs, dataset_fn, show_all, target_value):
    # shuffle_dataset(dataset_fn)
    training_data = Emb_Target_Dataset(csv_file="shuffled_dataset.csv", train=True, transform=ToTensor(), emb_id = epochs, target_column=target_value)
    test_data = Emb_Target_Dataset(csv_file="shuffled_dataset.csv", train=False, transform=ToTensor(), emb_id = epochs, target_column=target_value)
