...
class Emb_Target_Dataset(Dataset):
    ...
    def __init__(self, csv_file, train=True, transform=None, train_test_split=0.8, emb_id=0, target_column='outside_temp_Eindhoven', embedding_column='emb'):
        ...
        # print(min(self.emb_targets[self.target_column]))
        # print(max(self.emb_targets[self.target_column]))

        # target_min = min(self.emb_targets[self.target_column])
        # if target_min < 0:
        #     target_min = target_min * -1
        # target_max = max(self.emb_targets[self.target_column])

        # self.emb_targets[self.target_column] = (self.emb_targets[self.target_column] + target_min) / (target_max+target_min)
...
# ### test
# entities_file = "data/1res_1dev_111111010_uriTimeStamps_w_emb.csv"
# ds = Emb_Target_Dataset(entities_file)

# print(ds[900]['emb_targets'])
# print(ds.denormalize_single(ds[900]['emb_targets']))