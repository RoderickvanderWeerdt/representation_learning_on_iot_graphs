...
if __name__ == '__main__':
	OfficeGraph = True
	OfficeGraph_reverse = False
	opsd = False
	PecanStreet = False

	if OfficeGraph:
		# graph_name = "res1dev1TEMP_graph_111.ttl"
		target_class = "1_0_target"
		target_value = "outside_temp_Eindhoven"
		entities_file = "data/office/entities_bin.csv"
	...
	for graph_name in list_of_graph_names:
		if opsd: graph_name = "data/opsd/graphs/" + graph_name + ".nt"
		if PecanStreet: graph_name = "data/pecan/graphs/" + graph_name + ".nt"
		if OfficeGraph: graph_name = "data/office/graphs/" + graph_name + ".nt"
		if OfficeGraph_reverse: graph_name = "data/office_reverse/graphs/" + graph_name
		for i in range(0,3):
